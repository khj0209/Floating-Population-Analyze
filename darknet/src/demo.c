...
static int demo_json_port = 50000;
    ...
    int benchmark, int benchmark_layers)
{
    ...
    enum {
	    ...
	    TRUCK = 2,
	    CLASS_SIZE,
	    DELAY_SEC_TO_CLEAR = 6
    };

    unsigned int counts[CLASS_SIZE] = { 0, };
    unsigned int last_counts[CLASS_SIZE] = { 0, };

    printf("Start to update class counts...\n");

    int fd_server = socket(AF_INET, SOCK_STREAM, 0);
    struct sockaddr_in addr;
    addr.sin_family = AF_INET;
    addr.sin_port = ntohs(demo_json_port);
    ...
    if (connect(fd_server, (struct sockaddr *)&addr, sizeof(addr)) == -1) {
        ...
        ++count;
	{
		...
		unsigned int now_counts[CLASS_SIZE] = { 0, };

		if (difftime(now_time, last_time) >= DELAY_SEC_TO_CLEAR) {
            //send_json_my(CLASS_SIZE, names, counts, demo_json_port, 400000);
            char* json_string = detection_to_json_my(CLASS_SIZE, names, counts);
            int success = write(fd_server, json_string, strlen(json_string));
            if (success < 0) {
                printf("server writing error occurred.\n");
            } else {
                printf("send json : %s\n", json_string);
            }
            if (json_string != NULL) {
			...
			for (i = 0; i < nboxes; ++i) {
				for (class_id = 0; class_id < CLASS_SIZE; ++class_id) {
			...
			for (class_id = 0; class_id < CLASS_SIZE; ++class_id) {
				if (last_counts[class_id] < now_counts[class_id]) {
					counts[class_id] += now_counts[class_id] - last_counts[class_id];
				}
				last_counts[class_id] = now_counts[class_id];
			}
		}
