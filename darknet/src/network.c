...
char* detection_to_json_my(size_t class_size, char** names, unsigned int* classes_counts)
{
	time_t t  = time(NULL);
	...
	buf = (char*)malloc(96);
	if (buf != NULL) {
		sprintf(buf, "{{\"%04d-%02d-%02d\":{\"person\":%d,\"car\":%d,\"truck\":%d}}",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, classes_counts[0], classes_counts[1], classes_counts[2]);
	}
